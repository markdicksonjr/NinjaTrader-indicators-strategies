...
namespace NinjaTrader.NinjaScript.DrawingTools
{
	...
	/// </summary>
	public class RulerEnhanced : DrawingTool
	{
		...
		private string							yValueString, yValueString2, yValueUnit, yValueUnit2;
		private string 							timeText;
		private ValueUnit						yValueDisplayUnit,yValueDisplayUnit2			= ValueUnit.Price;
		...
		[Display(ResourceType = typeof(Custom.Resource), Name = "Second display unit", GroupName = "NinjaScriptGeneral", Order = 4)]
		...
		protected override void OnStateChange()
		{
			if (State == State.SetDefaults)
			{
				...
				YValueDisplayUnit = DrawingTools.ValueUnit.Ticks;
				YValueDisplayUnit2 = DrawingTools.ValueUnit.Currency;
			}
		...
		private void UpdateTextLayout(ChartControl chartControl, ChartPanel chartPanel, ChartScale chartScale)
		{
			...
			{
				case ValueUnit.Price	: 
					yValueString = chartBars.Bars.Instrument.MasterInstrument.FormatPrice(yDiffPrice);
					yValueUnit = "(price)";
					break;
				...
				case ValueUnit.Percent :
					yValueString = (yDiffPrice / AttachedTo.Instrument.MasterInstrument.RoundToTickSize(StartAnchor.Price)).ToString("P", Core.Globals.GeneralOptions.CurrentCulture);
					yValueUnit = "";
					break;
				case ValueUnit.Ticks	: 
					yValueString = yDiffTicks.ToString("F0"); 
					yValueUnit = "(ticks)";
					break;
				case ValueUnit.Pips		:
			...
			switch (YValueDisplayUnit2)
			{
				case ValueUnit.Price	: 
					yValueString2 = chartBars.Bars.Instrument.MasterInstrument.FormatPrice(yDiffPrice);
					yValueUnit2 = "(price)";
					break;
				case ValueUnit.Currency	: 
					yValueString2 = AttachedTo.Instrument.MasterInstrument.InstrumentType == InstrumentType.Forex
						? Core.Globals.FormatCurrency((int)(yDiffTicks) * Account.All[0].ForexLotSize * (AttachedTo.Instrument.MasterInstrument.TickSize * AttachedTo.Instrument.MasterInstrument.PointValue))
						: Core.Globals.FormatCurrency((int)(yDiffTicks) * (AttachedTo.Instrument.MasterInstrument.TickSize * AttachedTo.Instrument.MasterInstrument.PointValue)); 
					yValueUnit2 = "";
					break;
				case ValueUnit.Percent :
					yValueString2 = (yDiffPrice / AttachedTo.Instrument.MasterInstrument.RoundToTickSize(StartAnchor.Price)).ToString("P", Core.Globals.GeneralOptions.CurrentCulture);
					yValueUnit2 = "";
					break;
				case ValueUnit.Ticks	: 
					yValueString2 = yDiffTicks.ToString("F0"); 
					yValueUnit2 = "(ticks)";
					break;
				case ValueUnit.Pips		:
					// show tenth pips (if available)
					double pips = Math.Abs(yDiffTicks/10);
					char decimalChar = Char.Parse(Core.Globals.GeneralOptions.CurrentCulture.NumberFormat.NumberDecimalSeparator);
					yValueString2 = Int32.Parse(pips.ToString("F1").Split(decimalChar)[1]) > 0 ? pips.ToString("F1").Replace(decimalChar, '\'') : pips.ToString("F0");
					yValueUnit2 = "(pips)";
					break;
			}
			...
			string text = string.Format("{0}\n{1,-11}{2,-11}\n{3,-11}{4,-11}\n{5,-10}{6,-10}",
				...
				Custom.Resource.NinjaScriptDrawingToolRulerYValueText, yValueString + " " + yValueUnit + " " + yValueString2 + " " + yValueUnit2);
	...
	{
		private static Ruler RulerCoreEnhanced(NinjaScriptBase owner, string tag,  bool isAutoScale, int startBarsAgo, DateTime startTime, double startY, 
		...
		/// <returns></returns>
		public static Ruler RulerEnhanced(NinjaScriptBase owner, string tag, bool isAutoScale, int startBarsAgo, double startY, int endBarsAgo, double endY, int textBarsAgo, double textY)
		{
		...
		/// <param name="textY">The y value of the 3rd anchor point</param>
		/// <returns></returns>
		public static Ruler RulerEnhanced(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime startTime, double startY, DateTime endTime, double endY, DateTime textTime, double textY)
		{
		...
		/// <returns></returns>
		public static Ruler RulerEnhanced(NinjaScriptBase owner, string tag, bool isAutoScale, int startBarsAgo, double startY, int endBarsAgo, double endY, int textBarsAgo, double textY, bool isGlobal, string templateName)
		{
		...
		/// <param name="templateName">The name of the drawing tool template the object will use to determine various visual properties</param>
		/// <returns></returns>
		public static Ruler RulerEnhanced(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime startTime, double startY, DateTime endTime, double endY, DateTime textTime, double textY, bool isGlobal, string templateName)
		{
